...
// 导出产品类型
export function exportProductType(params?: ProductTypeQuery) {
  return request.get('/productType/export', { 
    params,
    responseType: 'blob'
  })
}
...
// 导入产品类型
export function importProductType(file: File) {
...
// 下载导入模板
export function downloadTemplateProductType() {
  return request.get('/productType/template', {
    responseType: 'blob'
  })
}
