...
func TestPreviewFileMigrationMarksPendingAndSkippedItems(t *testing.T) {
	db := testutil.SetupFileServiceTestDB(t)
	dir := t.TempDir()
	seedFileMigrationConfigs(t, dir)

	fakeMinio := testutil.NewFakeObjectStorageClient()
	resetBuilder := oss.RegisterClientBuilderForTest(model.StorageTypeMinio, func(storage *model.StorageProfile) (oss.Client, error) {
