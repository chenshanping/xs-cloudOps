...
func TestBuildChatCompletionRequestAddsSearchOptionsAndGuidanceWhenSearchEnabled(t *testing.T) {
	...
	if !req.EnableSearch {
		t.Fatal("expected enable_search to be true")
	}
	if req.SearchOptions == nil || !req.SearchOptions.ForcedSearch {
		t.Fatal("expected forced_search to be enabled when search is turned on")
	}
