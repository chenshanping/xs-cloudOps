...
func (s *StorageService) GetStorageByType(storageType model.StorageType) (*model.StorageProfile, error) {
	...
	configs, err := Config.GetConfigsByKeys([]string{
		StorageConfigKey(storageType),
		LegacyStorageConfigConfigKey,
	})
	if err != nil {
		return nil, err
	}
	...
	} else if legacyConfig, ok := configs[LegacyStorageConfigConfigKey]; ok {
...
func (s *StorageService) GetSystemStorage() (*model.StorageProfile, error) {
	configs, err := Config.GetConfigsByKeys([]string{StorageTypeConfigKey})
	if err != nil {
		return nil, err
	}

	storageType := s.DefaultStorageType()
	if config, ok := configs[StorageTypeConfigKey]; ok && strings.TrimSpace(config.Value) != "" {
...
func (s *StorageService) UpdateSystemStorage(storageType model.StorageType, configJSON string) error {
	...
	if err := Config.BatchUpdateConfigs(map[string]string{
		StorageTypeConfigKey:       string(storage.Type),
