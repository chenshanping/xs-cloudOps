...
func (s *DictService) UpdateDictType(id uint, req *request.UpdateDictTypeRequest) error {
	...
	if err == nil {
		// 清除旧类型和新类型的缓存
		_ = core.Default.ClearDictCache(oldType)
		if req.Type != oldType {
			_ = core.Default.ClearDictCache(req.Type)
		}
	}
	return err
}
...
func (s *DictService) DeleteDictType(id uint) error {
	...
	if err == nil {
		// 清除该字典类型的缓存
		_ = core.Default.ClearDictCache(typeCode)
	}
	return err
}
...
func (s *DictService) GetDictDataByType(dictType string) ([]model.SysDictData, error) {
	// 尝试从缓存获取
	if data, err := core.Default.GetDictFromCache(dictType); err == nil {
		var list []model.SysDictData
	...
	if data, err := json.Marshal(list); err == nil {
		_ = core.Default.SetDictToCache(dictType, data)
	}
...
func (s *DictService) CreateDictData(req *request.CreateDictDataRequest) error {
	...
	if err == nil {
		// 清除该字典类型的缓存
		_ = core.Default.ClearDictCache(req.DictType)
	}
	return err
}
...
func (s *DictService) UpdateDictData(id uint, req *request.UpdateDictDataRequest) error {
	...
	if err == nil {
		// 清除该字典类型的缓存
		_ = core.Default.ClearDictCache(dictType)
	}
	return err
}
