...
func (s *EmailService) GetEmailConfig() (*EmailConfig, error) {
	...
	configs, err := configsvc.Default.GetConfigsByKeys(keys)
	if err != nil {
		return nil, err
	}
...
func (s *EmailService) SendVerificationCode(to, code string) error {
	...
	if config, err := configsvc.Default.GetConfigByKey("sys_name"); err == nil && config.Value != "" {
...
func (s *EmailService) SendResetPasswordEmail(to, token string) error {
	...
	if config, err := configsvc.Default.GetConfigByKey("frontend_url"); err == nil && config.Value != "" {
