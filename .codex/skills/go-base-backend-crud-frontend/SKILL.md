---
name: go-base-backend-crud-frontend
description: Implement or refactor go-base admin CRUD modules, backend APIs, menu/button permissions, and matching Vue admin pages. Use when working in this repository on new admin pages, CRUD resources, page actions, menu/API bootstrap, or linked backend+frontend changes.
---
...
2. **Backend model + request** — create or update model in `server/model/<module>.go`, request types in `server/model/request/<module>_request.go`.
   ...
   - Request structs MUST declare boundary validation with `binding` tags where applicable: `required`, `min`, `max`, `email`, `oneof`, etc.
   - Use `comment` tags on request fields so validation errors can be translated into user-readable Chinese messages.
...
3. **Backend service** — create or update service in `server/service/<module>/`. Business logic in service, not handlers.
   ...
   - Service layer MUST handle business validation that cannot be safely expressed by `binding` tags alone, such as uniqueness, status transitions, permission preconditions, cross-field rules, and security policy checks.
...
4. **Backend API handlers** — create or update in `server/api/v1/<module>.go`. Use `response.BadRequest`, `response.Fail`, `response.OkWithData`, `response.OkWithPage`.
   - Handlers MUST bind request input before executing business logic: `ShouldBindJSON`, `ShouldBindQuery`, `ShouldBindUri`, etc.
   - Binding failures MUST use the shared friendly validation translator (for example `response.BindErrorMessage(err, req)`) instead of always returning `"参数错误"`.
   - Security-sensitive public endpoints may keep outward-facing errors intentionally vague when detailed messages would help enumeration or abuse. Do not leak account existence or internal security policy decisions unnecessarily.
...
13. **Self-check** — verify all items:
    ...
    - [ ] Request structs declare `binding` validation for required boundary fields?
    - [ ] Request fields have `comment` tags for friendly validation messages?
    - [ ] Handlers use `ShouldBind...` before business logic?
    - [ ] Binding failures use the shared friendly validation translator instead of a hard-coded `"参数错误"`?
    - [ ] Service layer covers business validation that `binding` tags cannot express?
    - [ ] Security-sensitive public endpoints avoid over-detailed error messages that enable enumeration or abuse?
...
- Boundary validation lives in request structs + handler binding; business validation lives in services
- Prefer shared validation-error translation over ad hoc hard-coded `"参数错误"`
